...
A Bitcoin P2P Taproot betting frontend. Users bet on binary outcomes (YES/NO) coordinated over Nostr, with funds locked in on-chain Bitcoin scripts resolved by oracle-revealed preimages.
...
- **Oracle** — creates a market (Kind 30050 event) committing to two SHA256 hashes (`yes_hash`, `no_hash`). Preimages stored in `localStorage`. Reveals one preimage at resolution blockheight. Market descriptions support Markdown.
...
### Contract script structure (`src/lib/contract.ts`)
...
Funded as Taproot Transaction. Winner claims by providing the oracle's revealed preimage + their signature.
...
maker_wallet_pubkey  x-only hex — used in contract script
taker_wallet_pubkey  x-only hex — used in contract script
maker_stake        sats
taker_stake        sats
```

Both wallet pubkeys are included so anyone can reconstruct the contract output scripts and verify the txid on-chain.
...
| `src/lib/contract.ts` | Contract script builder + `buildFundingTx` |
